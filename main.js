...
let loadObjectsMenu = async () => {
  for (let i = 0; i < objs.length; i++) {
    ...
    document
      .getElementById("canvas-objects-list")
      .appendChild(objs[i]["canvas"])
...
let handleSelectedObjOnScene = (onSceneId) => {
  if (selectedObjOnScene) {
    let previousSelected = document.getElementById(
      `menu-scene-item ${selectedObjOnScene}`
    )
...
async function renderSceneObjs(gl) {
  for (let i = 0; i < objsOnScene.length; i++) {
    if (objsOnScene[i] && objsOnScene[i].values.changed) {
      ...
      const objText = await loadFileContent(
        `${objPath}/${objsOnScene[i].fileOBJ}`
      )
      const obj = parseOBJ(objText)
      const mtlText = await loadFileContent(
        `${mtlPath}/${objsOnScene[i].fileMTL}`
      )
        ...
        Object.entries(material)
          ...
          .forEach(([key]) => {
            const texture = twgl.createTexture(gl, {
              src: texturePath + objsOnScene[i].texture,
              flipY: true,
            })
...
async function render() {
  if (objsOnScene.length != 0) {
    for (const objOnScene of objsOnScene) {
      if (
        objOnScene.values.cameraPosition &&
        objsData[objOnScene.fileOBJ + objOnScene.texture]
      ) {
        ...
        const camera = m4.lookAt(
          objOnScene.values.cameraPosition,
          objOnScene.values.cameraTarget,
          up
        )
        ...
        u_world = m4.multiply(
          m4.xRotation(objOnScene.values.rotationx),
          u_world
        )

        for (const { bufferInfo, vao, material } of objsData[
          objOnScene.fileOBJ + objOnScene.texture
        ]) {
...
const main = async () => {
  ...
  function changeRotationX() {
    if (selectedObjOnScene) {
      ...
      objsOnScene[index].values.rotationx = this.value
    } else alertEdit()
  }
  ...
  function changeRotationY() {
    if (selectedObjOnScene) {
      ...
      objsOnScene[index].values.rotationy = this.value
    } else alertEdit()
  }
  ...
  function changeSize() {
    if (selectedObjOnScene) {
      ...
      objsOnScene[index].values.size = this.value
      objsOnScene[index].values.changed = true
      renderSceneObjs(glScene)
    } else alertEdit()
  }
  ...
  inputRotationY.addEventListener("input", changeRotationY)
  inputRotationX.addEventListener("input", changeRotationX)
  ...
  function changeX() {
    if (selectedObjOnScene) {
      ...
      objsOnScene[index].values.x = parseFloat(this.value)
      objsOnScene[index].values.changed = true
      renderSceneObjs(glScene)
    } else alertEdit()
  }
  ...
  function changeY() {
    if (selectedObjOnScene) {
      ...
      objsOnScene[index].values.y = parseFloat(this.value)
      objsOnScene[index].values.changed = true
      renderSceneObjs(glScene)
    } else alertEdit()
  }
  ...
  function changeZ() {
    if (selectedObjOnScene) {
      ...
      objsOnScene[index].values.z = parseFloat(inputZ.value)
      objsOnScene[index].values.changed = true
      renderSceneObjs(glScene)
    } else alertEdit()
  }
  inputX.addEventListener("input", changeX)
  inputY.addEventListener("input", changeY)
  inputZ.addEventListener("input", changeZ)
